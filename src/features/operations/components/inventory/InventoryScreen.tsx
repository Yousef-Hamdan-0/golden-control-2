...
function movementReference( type: InventoryMovementType ) {
  ...
  if ( type === "sale" ) return "صرف مخزون";
  if ( type === "return" ) return "إرجاع مخزون";
  return "تسوية مخزون";
}
...
export function InventoryScreen( { section = "parts" }: { section?: string } ) {
  ...
  function adjustQuantity( partId: string, movementType: "supply" | "adjustment", quantity: number ) {
        ...
        cards={
          isMovement
            ? [
              ...
              {
                label: "حركات الصرف",
                value: String( inventoryMovements.filter( ( item ) => item.movementType === "sale" ).length ),
                icon: "box",
                tone: "gold",
              },
              {
