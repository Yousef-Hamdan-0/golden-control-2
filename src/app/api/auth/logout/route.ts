...
import { LogoutRequestSchema } from "@/models/auth/logout.model";
...
export async function POST(request: Request) {
  ...
  const parsedRequest = LogoutRequestSchema.safeParse(requestBody);
  ...
  try {
    const upstreamResponse = await fetch(BACKEND_API_ENDPOINTS.auth.logout, {
      method: "POST",
      headers: {
